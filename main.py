...
def get_recent_low(candles, lookback=10):
...
def is_buy_signal(candles): # Define factory. We are building the engine here, describing the conditions for a buy signal. We will use this function to determine if all conditions are met for a buy signal. 
    return (
        is_breakout(candles)
        and has_volume_confirmation(candles)
        and has_acceptable_rsi(candles, max_rsi=80)  # only change RSI in here, because we might want to use multiple RSI values for different strategies, so we can set the max_rsi value when calling the function
...
def calculate_trade_levels(candles, reward_multiplier=2):
    ...
    recent_low = get_recent_low(candles, lookback=10)
...
macd = calculate_macd(candles)
rsi = calculate_rsi(candles)
buy_signal = is_buy_signal(candles) # Store result = Use factory. This is where we call the function to check if all conditions are met for a buy signal, it is now stored in buy_signal, which is a boolean value (True or False)
...
print("Acceptable RSI:", has_acceptable_rsi(candles, max_rsi=80))
...
if buy_signal:
    ...
    if trade_levels is None:
        print("Not enough data for ATR-based trade.")
    else:
        print("Entry price:", round(trade_levels["entry_price"], 2))
        print("Stop loss:", round(trade_levels["stop_loss"], 2))
        print("Recent low:", round(trade_levels["recent_low"], 2))
        print("Take profit:", round(trade_levels["take_profit"], 2))
        print("Risk:", round(trade_levels["risk"], 2))
        print("ATR:", round(trade_levels["atr"], 4))
else:
    print("No trade.")

# always visible
atr = calculate_atr(candles, 14)
print("ATR (market):", round(atr, 4))